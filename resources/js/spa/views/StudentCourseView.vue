<template>
  <div class="student-course-view">
    ...
    <div v-show="activeTab === 'stream'">
      <StudentStreamTab :classId="classIdValue" />
    </div>
    <div v-show="activeTab === 'classwork'">
      <StudentClassworkTab :classId="classIdValue" :open-classwork-id="openClassworkId" />
    </div>
    <div v-show="activeTab === 'people'">
      <StudentPeopleTab :classId="classIdValue" />
    </div>
    <div v-show="activeTab === 'grades'">
      <StudentGradesTab :classId="classIdValue" />
    </div>
  </div>
</template>
...
import StudentStreamTab from '../components/StudentCourse/StreamTab.vue'
import StudentClassworkTab from '../components/StudentCourse/ClassworkTab.vue'
import StudentPeopleTab from '../components/StudentCourse/PeopleTab.vue'
import StudentGradesTab from '../components/StudentCourse/GradesTab.vue'
...
const openClassworkId = computed(() => route.query.classworkId || null)
...
function switchTab(tabKey) {
  ...
  // reflect in URL query (keep other query params like classworkId)
...
onMounted(async () => {
  ...
  activeTab.value = (route.query.tab && ['stream','classwork','people','grades'].includes(route.query.tab)) ? route.query.tab : 'stream'
...
watch(() => route.query.tab, (t) => {
  if (t && ['stream','classwork','people','grades'].includes(t)) activeTab.value = t
})
</script>
...
.student-course-view { padding-top: 0 }
